...
#include <string.h>
#include <locale.h>
int max(int a, int b){										//Ф-я максимума из двух
	if(a>=b) return a;
	return b;
	//(a >=b ) ? a : b
}
...
int BoyerMoore(char* str, char* substr){								//Ф-я Боера-Мура
	int Lsubstr, Lstr, i=0, k, j=Lsubstr-1, count=0, *stoptablearr, *suffdisparr, *suffarr;
	Lsubstr=strlen(substr); Lstr=strlen(str);
	stoptablearr=(int*)malloc(27*sizeof(int)); if(stoptablearr==NULL){printf("Ошибка. Не удалось выделить память."); exit(2);}
	...
	while(i<=(Lstr-Lsubstr)){
		...
		if(j==-1){
			if((int)str[i]==32){i+=max(1+j, suffdisparr[0]); printf("1\n");}
			else {i+=max(stoptablearr[(int)str[i]-97]-(Lsubstr-1-j), suffdisparr[0]); printf("1\n");}
		}
		else{
			if((int)str[i+j]==32){i+=max(1+j, suffdisparr[j]); printf("\n");}
...
int main(){
	...
	for(i=0;i<100;i++){
		...
		else if((int)c==32) str[i]=c;
		else if(c=='\n') {str[i]='\0'; break;}
		else{
			printf("\nОшибка. Допустимый алфафит: a-z + space.\n");
			free(substr);
			exit(1);
		}
	}
